...
bool load_content() {
	...
	for (unsigned int i = 0; i < MAX_PARTICLES; ++i) {
		positions[i] = vec4(((52.0f * dist(rand)) - 1.0f) / 10.0f, 5.0 * dist(rand), 0.0f, 0.0f);
...
bool render() {
	...
	glUniformMatrix4fv(eff.get_uniform_location("P"), 1, GL_FALSE, value_ptr(cam.get_projection()));
