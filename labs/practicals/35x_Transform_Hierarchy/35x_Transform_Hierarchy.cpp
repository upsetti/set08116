...
bool update(float delta_time) {
	...
	meshes[0].get_transform().rotate(vec3(0.0f, half_pi<float>(), 0.0f) * delta_time);
	// rotate Box Two on Z axis by delta_time
	meshes[1].get_transform().rotate(vec3(0.0f, 0.0f, half_pi<float>()) * delta_time);
	// rotate Box Three on Y axis by delta_time
	meshes[2].get_transform().rotate(vec3(0.0f, half_pi<float>(), 0.0f) * delta_time);
	// *********************************
	// Update the camera
